...
from app.crawler.schemas import PitcherGameLogData, PitcherStatData
...
PITCHER_DAILY_URL = f"{BASE_URL}/Record/Player/PitcherDetail/Daily.aspx?playerId={{player_id}}"
PITCHER_DETAIL_URL = f"{BASE_URL}/Record/Player/PitcherDetail/Basic.aspx?playerId={{player_id}}"
...
class KBOPitcherCrawler(BaseCrawler):
    ...
    def _parse_table(
        ...
        for tr in tbody.find_all("tr"):
            ...
            if not name or name in ("합계", "평균", "-"):
                continue
            link = tr.find("a", href=re.compile(r"playerId=\d+"))
            player_id_match = re.search(r"playerId=(\d+)", link.get("href", "")) if link else None
            ...
            results.append(
                PitcherStatData(
                    ...
                    team_code=team_code,
                    season=year,
                    kbo_player_id=player_id_match.group(1) if player_id_match else None,
    ...
    ) -> list[PitcherGameLogData]:
        """KBO 공식 일자별기록에서 해당 시즌 경기 로그를 반환."""
        soup = await self.fetch_html(PITCHER_DAILY_URL.format(player_id=player_id))
        logs: list[PitcherGameLogData] = []
        for table in soup.find_all("table"):
            headers = [th.get_text(strip=True) for th in table.find_all("th")]
            if "상대" not in headers or "구분" not in headers or "IP" not in headers:
                continue
            for tr in table.find_all("tr"):
                cells = [td.get_text(strip=True) for td in tr.find_all("td")]
                if len(cells) < 14 or not re.fullmatch(r"\d{2}\.\d{2}", cells[0]):
                    continue
                try:
                    game_date = datetime.strptime(f"{year}.{cells[0]}", "%Y.%m.%d").date()
                except ValueError:
                    continue
                logs.append(PitcherGameLogData(
                    kbo_player_id=player_id,
                    game_date=game_date,
                    opponent_name=cells[1],
                    role=cells[2],
                    game_result=cells[3] or None,
                    era=_to_float(cells[4]),
                    batters_faced=_to_int(cells[5]),
                    innings_pitched=_parse_innings(cells[6]),
                    hits=_to_int(cells[7]),
                    home_runs_allowed=_to_int(cells[8]),
                    walks=_to_int(cells[9]),
                    hbp=_to_int(cells[10]),
                    strikeouts=_to_int(cells[11]),
                    runs=_to_int(cells[12]),
                    earned_runs=_to_int(cells[13]),
                ))
        return logs

    async def fetch_player_injury_status(
        self, player_id: str, as_of: date
    ) -> str | None:
        """선수 상세의 등록 이력에서 현재 적용 중인 부상자명단 상태를 확인."""
        soup = await self.fetch_html(PITCHER_DETAIL_URL.format(player_id=player_id))
        for tr in soup.find_all("tr"):
            cells = [td.get_text(" ", strip=True) for td in tr.find_all("td")]
            if len(cells) < 3 or "부상자명단" not in " ".join(cells):
                continue
            try:
                start = datetime.strptime(cells[0], "%Y-%m-%d").date()
                end = datetime.strptime(cells[1], "%Y-%m-%d").date()
            except ValueError:
                continue
            if start <= as_of <= end:
                return f"부상자명단 ({start:%m.%d}~{end:%m.%d})"
        return None
...
async def run_pitcher_game_logs(
    stats: list[PitcherStatData], year: int, as_of: date | None = None
) -> tuple[list[PitcherGameLogData], dict[str, str | None]]:
    """시즌 투수별 경기 로그와 현재 부상자명단 상태 수집."""
    target_date = as_of or today_kst()
    logs: list[PitcherGameLogData] = []
    ...
    async with make_client() as client:
        ...
        async def fetch_details(
            stat: PitcherStatData,
        ) -> tuple[str | None, list[PitcherGameLogData], str | None]:
            player_id = stat.kbo_player_id
            if not player_id:
                return None, [], None
            async with semaphore:
                player_logs: list[PitcherGameLogData] = []
                injury: str | None = None
                try:
                    player_logs = await crawler.fetch_player_game_logs(player_id, year)
                except Exception as e:
                    logger.error("Failed to fetch pitcher logs for %s(%s): %s", stat.player_name, player_id, e)
                try:
                    injury = await crawler.fetch_player_injury_status(player_id, target_date)
                except Exception as e:
                    logger.error("Failed to fetch pitcher injury for %s(%s): %s", stat.player_name, player_id, e)
                return player_id, player_logs, injury

        for player_id, player_logs, injury in await asyncio.gather(
            *(fetch_details(stat) for stat in stats)
        ):
            if not player_id:
                continue
