      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        {[
          ...
          <Link key={m.href} href={m.href}
            className="rounded-xl p-3 text-center transition-all duration-200 hover:border-indigo-500/30 hover:bg-[#162033] sm:p-4"
          ...
          <div className="w-full lg:w-60 shrink-0">
            <div className="rounded-2xl overflow-hidden"
              ...
              <div className="px-4 py-3 flex items-center justify-between border-b border-white/[0.05]">
                <h2 className="text-xs font-black tracking-wider uppercase text-slate-400">팀 순위</h2>
                <Link href="/teams" className="text-xs font-medium text-indigo-400 hover:text-indigo-300 transition-colors">
                  전체 보기
                </Link>
              </div>
              <div>
                {top5.map((t) => (
                  <Link key={t.id} href={`/teams/${t.id}`}
                    className="flex items-center px-4 py-2.5 border-b border-white/[0.03] hover:bg-indigo-500/5 transition-colors">
                    <span className={`w-5 text-center text-xs font-black mr-3 ${
                      t.rank === 1 ? "text-yellow-400" : t.rank === 2 ? "text-slate-300" : t.rank === 3 ? "text-amber-600" : "text-slate-600"
                    }`}>{t.rank}</span>
                    <span className="flex-1 text-sm font-bold text-slate-200">{t.short_name ?? t.name}</span>
                    <span className="text-xs tabular-nums text-slate-500">{t.wins}승</span>
                    <span className={`ml-2 text-xs font-bold tabular-nums ${t.win_rate >= 0.5 ? "text-emerald-400" : "text-slate-600"}`}>
              ...
              <div className="px-4 py-2.5 border-t border-white/[0.04]">
                <Link href="/teams" className="text-xs text-slate-600 hover:text-slate-400 transition-colors">
                  하위 5팀 보기 →
                </Link>
              </div>
            </div>
