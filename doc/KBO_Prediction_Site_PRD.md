...
┌─────────────────────────────────────────────────────┐
│                    데이터 소스                         │
│  Statiz.co.kr  │  KBO 공식  │  Open-Meteo  │  네이버뉴스  │
└────────┬───────┴─────┬──────┴──────┬───────┴────┬────┘
         │             │             │             │
         ▼             ▼             ▼             ▼
┌─────────────────────────────────────────────────────┐
│           Crawler / Scheduler (Python)               │
...
┌─────────────────────────────────────────────────────┐
│              분석 엔진 (Python)                        │
│   ELO 레이팅  │  투수 보정  │  최근 흐름  │  날씨 보정    │
└──────────────────────┬──────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────┐
│               FastAPI 서버                            │
│          REST API + JWT 인증 + CORS                  │
└──────────────────────┬──────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────┐
...
- [ ] Statiz 팀 스탯 크롤러
- [ ] Statiz 투수 스탯 크롤러
- [ ] APScheduler 자동 실행 설정
- [ ] 2025 시즌 과거 데이터 수집 (백테스팅용)

### Phase 2 — 예측 엔진 (2~3주)

- [ ] ELO 레이팅 초기화 및 히스토리 계산
- [ ] 선발 투수 보정 로직
- [ ] 최근 흐름 계산 로직
- [ ] 복합 예측 모델 (가중치 조합)
- [ ] 백테스팅 — 2025 시즌 데이터로 적중률 검증
- [ ] Redis 캐싱 연동
...
- [ ] Next.js 프로젝트 설정 (App Router + TypeScript)
- [ ] 메인 페이지 (오늘의 경기 + 예측 카드)
- [ ] 팀 순위 페이지
- [ ] 팀 상세 / 투수 상세 페이지
- [ ] 예측 히스토리 페이지
